...
class MyCustomSemiBoldTV : AppCompatTextView {
    ...
    private fun selectTypeface(context: Context, fontName: String?, textStyle: Int): Typeface? {
        return if (fontName == null) {
            FontCache.getTypeface("outfit_black.ttf", context)
        } else if (fontName.contentEquals("outfit_black.ttf")) {
            FontCache.getTypeface("outfit_black", context)
        } else {
            FontCache.getTypeface("outfit_black.ttf", context)
        }
    }
