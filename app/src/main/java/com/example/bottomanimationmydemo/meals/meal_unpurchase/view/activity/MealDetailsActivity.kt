package com.example.bottomanimationmydemo.meals.meal_unpurchase.view.activity
...
import com.example.bottomanimationmydemo.databinding.AlreadyMealSubscribeDialogBinding
...
import com.example.bottomanimationmydemo.view.activity.LoginActivity
...
//            startActivity(Intent(this@MealDetailsActivity, RegistrationActivity::class.java).putExtra("batchMeal", "BatchMeal"))
    ...
    private fun checkMealSubscribeStatus(user_id:String,meal_id:String) {
        if (CheckNetworkConnection.isConnection(this, binding.root, true)) {
            ...
            authViewModel.mealSubscribeCheckApiCall(jsonObject)
            authViewModel.mealCheckSubscribeModelResponse.observe(this) {
                when (it) {
                    is Resource.Success -> {
                        hideLoader()
                        authViewModel.mealCheckSubscribeModelResponse.removeObservers(this)
                        if (authViewModel.mealCheckSubscribeModelResponse.hasObservers()) return@observe
                        lifecycleScope.launch {
                            it.let {
                                val response = it.value
                                if (response.status == MyConstant.success) {
